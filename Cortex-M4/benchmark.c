...
/* maximum runtime for each benchmark */
    ...
    {
        printf("DEBUG: aes_aad_options = %d\n", aes_aad_options);
...
void bench_sha256_custom(int useDeviceID)
{
    WC_DECLARE_ARRAY(hash, wc_Sha256, BENCH_MAX_PENDING,
                     sizeof(wc_Sha256), HEAP_HINT);
    double start, end, init_time = 0, update_time = 0, final_time = 0;
...
void bench_sha256_custom_init(int useDeviceID)
{
    WC_DECLARE_ARRAY(hash, wc_Sha256, BENCH_MAX_PENDING,
                     sizeof(wc_Sha256), HEAP_HINT);
    double start, end;
    double sha_init_time = 0;
    int    ret = 0, i, count = 0, times;
    DECLARE_MULTI_VALUE_STATS_VARS()
    WC_DECLARE_ARRAY(digest, byte, BENCH_MAX_PENDING,
                     WC_SHA256_DIGEST_SIZE, HEAP_HINT);
	...
	printf("SHA-256-Init took %.3f us\n", (end-start)/count*1e6);
...
void bench_sha256_custom_update(int useDeviceID)
{
    WC_DECLARE_ARRAY(hash, wc_Sha256, BENCH_MAX_PENDING,
                     sizeof(wc_Sha256), HEAP_HINT);
    double start, end;
	...
	printf("SHA-256-Update took %.3f us\n", (end-start)/count*1e6);
...
void bench_sha256_custom_final(int useDeviceID)
{
    WC_DECLARE_ARRAY(hash, wc_Sha256, BENCH_MAX_PENDING,
                     sizeof(wc_Sha256), HEAP_HINT);
    double start, end;
	...
	printf("SHA-256-Final took %.3f us\n", (end-start)/count*1e6);
...
#ifdef WOLFSSL_SHA512
void bench_sha512_custom(int useDeviceID)
{
...
#ifdef HAVE_POLY1305
void bench_poly1305_custom(void)
{
    Poly1305 enc;
    byte     mac[16];
    double   start, end;
...
#if defined(HAVE_CHACHA) && defined(HAVE_POLY1305)
void bench_chacha20_poly1305_aead_custom(void)
{
    double start, end;
    int    ret = 0, i, count;
...
#ifdef HAVE_AESGCM
void bench_aesgcm_custom(int useDeviceID);
#endif

#ifdef HAVE_AES_CBC
void bench_aescbc_custom(int useDeviceID);
#endif

#if defined(HAVE_BLAKE2S)
void bench_blake2s_custom(void)
{
    Blake2s b2s;
    byte    digest[32];
    double  start, end;
...
#ifdef HAVE_BLAKE2
void bench_blake2b_custom(void)
{
    Blake2b b2b;
    byte    digest[64];
    double  start, end;
...
static void* benchmark_graphene()
{

#ifdef HAVE_AES_CBC
    if (bench_all || (bench_cipher_algs & BENCH_AES_CBC)) {
    #ifndef NO_SW_BENCH
        bench_aescbc_custom(0);
    #endif
    #if defined(BENCH_DEVID)
        bench_aescbc_custom(1);
    #endif
    }
#endif
...
#ifndef NO_SW_BENCH
        bench_aes_aad_options_wrap(bench_aesgcm_custom, 0);
#endif
...
//     bench_aes_aad_options_wrap(bench_aesgcm_custom, 1);
// #endif
}
#endif
    ...
    for (int i = 0 ; i < 13 ; i++) {
        bench_size = 1 << i;
        bench_sha512_custom(0);
    }
    printf("\n");

    bench_hmac_sha256_custom(0);
#endif
    ...
    for (int i = 0 ; i < 13 ; i++) {
        bench_size = 1 << i;
        bench_chacha20_poly1305_aead_custom();
    }
    printf("\n");
#endif
...
static void bench_mp(int modulusSize, int mode)
{
    WC_RNG rng;
    double start, end;
    int count, times, ret;
    char decStr[1024] = "";
    byte binStr[1024] = {};
    unsigned char inBin[modulusSize*4];
    mp_int sk_a, sk_b, sk_mod, out, in, aux;

    static char opLabels[][25] = {
        "add", "sub", "mul", "sqr", "mod","addmod", "submod", "mulmod", "sqrmod", "mod-read"
    };
    ...
    ret = mp_init_multi(&sk_a, &sk_b, &sk_mod, &out, &in, &aux);
    if (ret != 0) {
        printf("ERROR: mp_init_multi failed!\n");
        return;
    }

    if (mp_rand(&sk_a, modulusSize, &rng) != MP_OKAY &
        mp_rand(&sk_b, modulusSize, &rng) != MP_OKAY &
        mp_rand(&in, modulusSize, &rng) != MP_OKAY &
        mp_rand_prime(&sk_mod, modulusSize*4, &rng, NULL) != MP_OKAY
    ) {
        printf("DEBUG: MP_RAND failed!\n");
        return -1;
    }

    ret = wc_RNG_GenerateBlock(&rng, inBin, sizeof(inBin));
    if (ret != 0) {
        printf("ERROR: wc_RNG_GenerateBlock failed!\n");
        return -1;
    }

    mp_read_unsigned_bin(&in, inBin, sizeof(inBin));
    ...
    do {
        for (times = 0; times < numBlocks; times++) {
            ...
            {
            case 0:
                if (mp_add(&sk_b, &sk_b, &out) != MP_OKAY) {
                    printf("Error: mp_add failed\n");
                    goto mp_exit;
                }
                break;
            case 1:
                if (mp_sub(&sk_a, &sk_b, &out) != MP_OKAY) {
                    printf("Error: mp_sub failed\n");
                    goto mp_exit;
                }
                break;
            case 2:
                if (mp_mul(&sk_a, &sk_b, &out) != MP_OKAY) {
                    printf("Error: mp_mul failed\n");
                    goto mp_exit;
                }
                break;
            case 3:
                if (mp_sqr(&sk_a, &out) != MP_OKAY) {
                    printf("Error: mp_sqr failed\n");
                    goto mp_exit;
                }
                break;
            case 4:
                if (mp_mod(&in, &sk_mod, &out) != MP_OKAY) {
                    printf("Error: mp_mod failed\n");
                    goto mp_exit;
                }
                break;
            case 5:
                if (mp_addmod(&sk_a, &sk_b, &sk_mod, &out) != MP_OKAY) {
                    printf("Error: mp_addmod failed\n");
                    goto mp_exit;
                }
                break;
            case 6:
                if (mp_submod(&sk_a, &sk_b, &sk_mod, &out) != MP_OKAY) {
                    printf("Error: mp_submod failed\n");
                    goto mp_exit;
                }
                break;
            case 7:
                if (mp_mulmod(&sk_a, &sk_b, &sk_mod, &out) != MP_OKAY) {
                    printf("Error: mp_mulmod failed\n");
                    goto mp_exit;
                }
                break;
            case 8:
                if (mp_sqrmod(&sk_a, &sk_mod, &out) != MP_OKAY) {
                    printf("Error: mp_sqrmod failed\n");
                    goto mp_exit;
                }
                break;
            case 9:
    ...
    printf("Bench_MP(%3d, %s) took %3.3f us\n", modulusSize*32, opLabels[mode], (end-start)/count*1e6);

    // mp_todecimal(&sk_a, decStr);
    // printf("sk_a = %s\n", decStr);
    // mp_todecimal(&sk_b, decStr);
    // printf("sk_b = %s\n", decStr);
    // mp_todecimal(&sk_mod, decStr);
    // printf("sk_m = %s\n", decStr);
    // mp_todecimal(&in, decStr);
    // printf("in   = %s\n", decStr);
    // mp_todecimal(&out, decStr);
    // printf("out  = %s\n", decStr);
...
/*
 * bench_lcumac
 * modulusSize: Size of the modulus in (words)
*/
static void bench_lcumac(int modulusSize)
{
    WC_RNG rng;
    double start, end;
    int count, times, ret;
    ...
    byte a_str[16], b_str[16], out_str[8*8][16];
    unsigned char inBin[modulusSize*4];
    mp_int sk_a, sk_b, sk_mod, out, in, aux;
    ...
    do {
        for (times = 0; times < numBlocks; times++) {
            ...
            if (mp_read_unsigned_bin(&in, inBin, sizeof(inBin)) != MP_OKAY) {
                printf("Error: mp_read_unsigned_bin failed\n");
                goto lcumac_exit;
            }

            if (mp_mul(&sk_a, &in, &aux) != MP_OKAY) {
                printf("Error: mp_mul failed\n");
                goto lcumac_exit;
            }

            if (mp_addmod(&aux, &sk_b, &sk_mod, &out) != MP_OKAY) {
                printf("Error: mp_add failed\n");
                goto lcumac_exit;
            }
    ...
    printf("Bench_LCUMAC(%3d) took %3.3f us\n", modulusSize*32, (end-start)/count*1e6);
 ...
 * modulusSize: Size of the modulus in (words)
*/
static void bench_lcumac_custom(int modulusSize, int inlen)
{
    WC_RNG rng;
    double start, end;
    int count, times, ret;
    ...
    do {
        for (times = 0; times < numBlocks; times++) {
            ...
            for (int chunk = 0 ; chunk < MAX(1, inlen / (modulusSize * 4)) ; chunk++) {

                if (mp_read_unsigned_bin(&in, inBin, modulusSize * 4) != MP_OKAY) {
                    printf("Error: mp_read_unsigned_bin failed\n");
                    goto lcumac_exit;
                }
    ...
    printf("Bench_LCUMAC(|q|=%3d, |m|=%4d) took %3.3f us\n", modulusSize*32, inlen, (end-start)/count*1e6);
...
/*
 * bench_sqhumac
 * modulusSize: modulus size (per word of 4 bytes)
*/
static void bench_sqhumac(int modulusSize)
{
    WC_RNG rng;
    double start, end;
    int count, times, ret;
    ...
    ret = mp_init_multi(&sk_a, &sk_mod, &out, &in, &aux, NULL);
    if (ret != 0) {
        printf("ERROR: mp_init_multi failed!\n");
        return;
    }
    ...
    do {
        for (times = 0; times < numBlocks; times++) {
            ...
            if (mp_add(&sk_a, &in, &aux) != MP_OKAY) {
                printf("Error: mp_mul failed\n");
                goto lcumac_exit;
            }

            if (mp_sqrmod(&aux, &sk_mod, &out) != MP_OKAY) {
                printf("Error: mp_add failed\n");
                goto lcumac_exit;
            }
    ...
    printf("Bench_SQHUMAC(%3d) took %3.3f us\n", modulusSize*32, (end-start)/count*1e6);
...
*/
static void bench_sqhumac_custom(int modulusSize, int inlen)
{
    WC_RNG rng;
    double start, end;
    int count, times, ret;
    ...
    printf("Bench_SQHUMAC(|q|=%3d, |m|=%4d) took %3.3f us\n", modulusSize*32, inlen, (end-start)/count*1e6);
...
static void bpmac_auth(unsigned char in[], int inlen, unsigned char *bitflips[]) {
    uint8_t out[16];
    int i, ii, bit, index = 0;
    ...
    for (i = 0 ; i < inlen ; i++) {
        for (bit = 0 ; bit < 8 ; bit++) {
            ...
            for (ii = 0 ; ii < 16 ; ii++) {
                ...
                out[ii] = out[ii] ^ bitflips[index][ii];

                out[ii] = out[ii] & bitflips[index][ii];

            }

            // ((uint64_t*)out)[0] = ((uint64_t*)(bitflips[index]))[0] ^ ((uint64_t*)out)[0];
            // ((uint64_t*)out)[1] = ((uint64_t*)(bitflips[index]))[1] ^ ((uint64_t*)out)[1];

            xor_tags(out, bitflips[index]);
        }
    }
}


static void bench_bpmac(int numbits)
{
    WC_RNG rng;
    double start, end;
    int count, times, ret;
    byte in[numbits / 8], tag[MAC_LEN], bitflips[numbits][MAC_LEN];
    ...
    ret = wc_RNG_GenerateBlock(&rng, in, sizeof(in));
    if (ret != 0) {
        printf("ERROR: wc_RNG_GenerateBlock failed!\n");
        return -1;
    }
    ...
    for (int i = 0 ; i < numbits ; i++) {
        ret = wc_RNG_GenerateBlock(&rng, bitflips[i], sizeof(bitflips[i]));
        if (ret != 0) {
            printf("ERROR: wc_RNG_GenerateBlock failed!\n");
            return -1;
        }
    ...
    do {
        for (times = 0; times < numBlocks; times++) {
            ...
            for (int bit_index = 0 ; bit_index < numbits ; bit_index++) {

                xor_tags(tag, bitflips[bit_index]);

                // for (int i = 0 ; i < MAC_LEN ; i++) {

                //     tag[i] = tag[i] ^ bitflips[bit_index][i];
                // }
            }
    ...
    printf("Bench_BPMAC(bytes=%3d) took %4.3f us\n", numbits / 8, (end-start)/count*1e6);
}



static void bench_xor(int numbytes)
{
    WC_RNG rng;
    double start, end;
    int count, times, ret;
    byte in_a[numbytes], in_b[numbytes], out[numbytes];
    ...
    ret = wc_RNG_GenerateBlock(&rng, in_a, sizeof(in_a));
    if (ret != 0) {
        printf("ERROR: wc_RNG_GenerateBlock failed!\n");
        return -1;
    }

    ret = wc_RNG_GenerateBlock(&rng, in_b, sizeof(in_b));
    if (ret != 0) {
        printf("ERROR: wc_RNG_GenerateBlock failed!\n");
        return -1;
    }
    ...
    if (numbytes >= 8) {
        do {
            for (times = 0; times < numBlocks; times++) {
                ...
                for (int byte_index = 0 ; byte_index < numbytes / 8 ; byte_index++) {

                    ((uint64_t *) out)[byte_index] = ((uint64_t *) in_a)[byte_index] ^ ((uint64_t *) in_b)[byte_index];
        ...
        do {
                for (times = 0; times < numBlocks; times++) {
                    ...
                    for (int byte_index = 0 ; byte_index < numbytes / 4 ; byte_index++) {

                        ((uint32_t *) out)[byte_index] = ((uint32_t *) in_a)[byte_index] ^ ((uint32_t *) in_b)[byte_index];
        ...
        do {
            for (times = 0; times < numBlocks; times++) {
                ...
                for (int byte_index = 0 ; byte_index < numbytes ; byte_index++) {

                    out[byte_index] = in_a[byte_index] ^ in_b[byte_index];
    ...
    printf("Bench_XOR(bytes=%3d) took %4.3f us\n", numbytes, (end-start)/count*1e6);
}


static void benchmark_umac(void)
{
...
//     bench_size = 1024;
    ...
    for (int numbytes = 1 ; numbytes <= 4096 ; numbytes *= 2) {
        bench_xor(numbytes);
    }
    printf("\n");

    // for (int numbytes = 1 ; numbytes <= 256 ; numbytes *= 2) {
    //     bench_bpmac(numbytes * 8);
    // }
    // printf("\n");

    for (int numdigits = 2 ; numdigits < 9 ; numdigits++) {
        bench_lcumac(numdigits);
    }
    printf("\n");

    for (int numdigits = 2 ; numdigits < 9 ; numdigits++) {
        bench_sqhumac(numdigits);
    }
    printf("\n");

    for (int numbytes = 1 ; numbytes <= 4096 ; numbytes *= 2) {
        bench_lcumac_custom(4, numbytes);
    }
    printf("\n");

    for (int numbytes = 1 ; numbytes <= 4096 ; numbytes *= 2) {
        bench_lcumac_custom(8, numbytes);
    }
    printf("\n");

    for (int numbytes = 1 ; numbytes <= 4096 ; numbytes *= 2) {
        bench_sqhumac_custom(4, numbytes);
    }
    printf("\n");

    for (int numbytes = 1 ; numbytes <= 4096 ; numbytes *= 2) {
        bench_sqhumac_custom(8, numbytes);
    }
    printf("\n");

    // for (int numdigits = 2 ; numdigits < 9 ; numdigits++) {
    //     for (int mode = 0 ; mode <= 9 ; mode++) {
    //         bench_mp(numdigits, mode);
    //     }
    //     printf("\n");
    // }
}
...
static void* benchmarks_do(void* args)
{
    ...
    if (bench_all || (bench_cipher_algs & BENCH_AES_CBC)) {
    #ifndef NO_SW_BENCH
        printf("AES CBC software benchmark\n");
        bench_aescbc(0);
    #endif
    #if defined(BENCH_DEVID)
        printf("AES CBC hardware benchmark\n");
        bench_aescbc(1);
    #endif
    }
#endif
    ...
    #ifndef NO_SW_BENCH
        bench_gmac(0);
        printf("AES GMAC software benchmark\n");
        for (int i = 0 ; i < 13 ; i++) {
            bench_size = 1 << i;
            bench_gmac_custom(0);
        }
   ...
   (!defined(HAVE_FIPS) || FIPS_VERSION_GE(5, 3)) && !defined(HAVE_SELFTEST)
    ...
    #ifndef NO_SW_BENCH
        bench_rsa(0);
        printf("DEBUG: rsa sw bench..\n");
    #endif
    #ifdef BENCH_DEVID
        bench_rsa(1);
        printf("DEBUG: rsa hw bench..\n");
    #endif
    }
    ...
    if (bench_all || (bench_asym_algs & BENCH_ECC_MAKEKEY) ||
        ...
        if (bench_asym_algs & BENCH_ECC_ALL) {
            ...
            #ifndef NO_ECC256
            printf("bench ecc curve ECC_SECP256R1...\n");
            bench_ecc_curve((int)ECC_SECP256R1);
...
static void bench_aescbc_internal_stream_custom(int useDeviceID,
                                  ...
                                  const char* decLabel)
{
    ...
    if ((ret = wc_AesInit(enc[0], HEAP_HINT,
                        useDeviceID ? devId: INVALID_DEVID)) != 0) {
        printf("AesInit failed at L%d, ret = %d\n", __LINE__, ret);
        goto exit;
    }

    ret = wc_AesSetKey(enc[0], key, keySz, iv, AES_ENCRYPTION);
    if (ret != 0) {
    ...
    do {
        for (times = 0; times < numBlocks; times++) {
            ret = wc_AesCbcEncrypt(enc[0], out, in, bench_size);
            if (ret != 0) {
                printf("AesCbcEncrypt failed, ret = %d\n", ret);
                goto exit;
            }
        } /* for times */
        count += times;
...
exit_aes_enc:
    ...
    do {
        for (times = 0; times < numBlocks; times++) {
            ret = wc_AesCbcDecrypt(enc[0], out, in, bench_size);
            if (ret != 0) {
                printf("AesCbcDecrypt failed, ret = %d\n", ret);
                goto exit;
            }
        } /* for times */
        count += times;
...
exit_aes_dec:
	...
	// printf("%s-Stream took %.3f us\n", decLabel, (end-start)/count*1e6);
...
static void bench_aescbc_internal_custom(int useDeviceID,
                                  const byte* key, word32 keySz,
                                  const byte* iv, const char* encLabel,
                                  const char* decLabel)
{
...
void bench_aescbc_custom(int useDeviceID)
{
...
#ifdef HAVE_RENESAS_SYNC
    bench_aescbc_internal_custom(useDeviceID, bench_key1, 16, bench_iv,
                 "AES-128-CBC-enc", "AES-128-CBC-dec");
#else
    printf("bench_aescbc_128: \n");
    for (int i = 0 ; i < 14 ; i++) {
        bench_size = 1 << i;
        bench_aescbc_internal_custom(useDeviceID, bench_key, 16, bench_iv,
                    "AES-128-CBC-enc", "AES-128-CBC-dec");
    }
    printf("\n");

    printf("bench_aescbc_128_stream: \n");
    for (int i = 0 ; i < 13 ; i++) {
        bench_size = 1 << i;
        bench_aescbc_internal_stream_custom(useDeviceID, bench_key, 16, bench_iv,
                    "AES-128-CBC-enc", "AES-128-CBC-dec");
    }
    printf("\n");
#endif
    ...
    // printf("bench_aescbc_192: \n");
    // for (int i = 0 ; i < 13 ; i++) {
    //     bench_size = 1 << i;
    //     bench_aescbc_internal_custom(useDeviceID, bench_key, 24, bench_iv,
    //              "AES-192-CBC-enc", "AES-192-CBC-dec");
    // }
    // printf("\n");

    // for (int i = 0 ; i < 13 ; i++) {
    // printf("bench_aescbc_192_stream: \n");
    //     bench_aescbc_internal_stream_custom(useDeviceID, bench_key, 24, bench_iv,
    //              "AES-192-CBC-enc", "AES-192-CBC-dec");
    // }
    // printf("\n");
#endif
...
#ifdef HAVE_RENESAS_SYNC
    bench_aescbc_internal_custom(useDeviceID, bench_key2, 32, bench_iv,
                 "AES-256-CBC-enc", "AES-256-CBC-dec");
#else
    // printf("bench_aescbc_256: \n");
    // for (int i = 0 ; i < 13 ; i++) {
    //     bench_size = 1 << i;
    //     bench_aescbc_internal_custom(useDeviceID, bench_key, 32, bench_iv,
    //              "AES-256-CBC-enc", "AES-256-CBC-dec");
    // }
    // printf("\n");

    // printf("bench_aescbc_256_stream: \n");
    // for (int i = 0 ; i < 13 ; i++) {
    //     bench_aescbc_internal_stream_custom(useDeviceID, bench_key, 32, bench_iv,
    //              "AES-256-CBC-enc", "AES-256-CBC-dec");
    // }
    // printf("\n");
#endif
...
static void bench_aesgcm_internal_custom(int useDeviceID,
                                  const byte* key, word32 keySz,
                                  const byte* iv,  word32 ivSz,
                                  const char* encLabel, const char* decLabel)
{
    ...
    do {
        for (times = 0; times < numBlocks; times++) {
            ...
            // printf("DEBUG: we are here\n");
            ret = wc_AesGcmEncrypt(enc[0], bench_cipher,
                    bench_plain, bench_size,
                    iv, ivSz, bench_tag, AES_AUTH_TAG_SZ,
                    bench_additional, aesAuthAddSz);
            if (ret != 0) {
                printf("AesGcmEncrypt failed, ret = %d\n", ret);
                goto exit;
            }
        } /* for times */
        count += times;
...
#undef HAVE_AES_DECRYPT
    ...
    do {
        for (times = 0; times < numBlocks; times++) {
            if ((ret = wc_AesInit(dec[0], HEAP_HINT,
                        useDeviceID ? devId: INVALID_DEVID)) != 0) {
                printf("AesInit failed at L%d, ret = %d\n", __LINE__, ret);
                goto exit;
            }

            ret = wc_AesGcmSetKey(dec[0], key, keySz);
            if (ret != 0) {
                printf("AesGcmSetKey failed, ret = %d\n", ret);
                goto exit;
            }

            ret = wc_AesGcmDecrypt(dec[0], bench_plain,
                        bench_cipher, bench_size,
                        iv, ivSz, bench_tag, AES_AUTH_TAG_SZ,
                        bench_additional, aesAuthAddSz);
            if (ret != 0) {
                printf("AesGcmDecrypt failed, ret = %d\n", ret);
                goto exit;
            }
        } /* for times */
        count += times;
...
exit_aes_gcm_dec:
	...
	// printf("%.3f & ", decLabel, (end-start)/count*1e6);
...
#else
    printf("%s-custom: (useDeviceID=%d) \n", AES_GCM_STRING(128, enc), useDeviceID);
    for (int i = 0 ; i < 14 ; i++) {
        bench_size = 1 << i;
        bench_aesgcm_internal_custom(useDeviceID, bench_key, 16, bench_iv, 12,
                        AES_GCM_STRING(128, enc), AES_GCM_STRING(128, dec));
    }
    printf("\n");

    // printf("%s-custom: \n", AES_GCM_STRING(192, enc));
    // for (int i = 0 ; i < 13 ; i++) {
    //     bench_size = 1 << i;
    //     bench_aesgcm_internal_custom(useDeviceID, bench_key, 24, bench_iv, 12,
    //                     AES_GCM_STRING(192, enc), AES_GCM_STRING(192, dec));
    // }
    // printf("\n");

    // printf("%s-custom: \n", AES_GCM_STRING(256, enc));
    // for (int i = 0 ; i < 13 ; i++) {
    //     bench_size = 1 << i;
    //     bench_aesgcm_internal_custom(useDeviceID, bench_key, 32, bench_iv, 12,
...
/* GMAC */
void bench_gmac_custom(int useDeviceID)
{
    int ret, count = 0;
    Gmac gmac;
    double start, end;
...
static void bench_hmac_custom(int useDeviceID, int type, int digestSz,
                       const byte* key, word32 keySz, const char* label)
{
    WC_DECLARE_ARRAY(hmac, Hmac, BENCH_MAX_PENDING,
                     sizeof(Hmac), HEAP_HINT);
    ...
    do {
            for (times = 0; times < numBlocks; times++) {
                ret = wc_HmacInit(hmac[0], HEAP_HINT,
                    useDeviceID ? devId : INVALID_DEVID);
                if (ret != 0) {
                    printf("wc_HmacInit failed for %s, ret = %d\n", label, ret);
                    goto exit;
                ...
                ret = wc_HmacSetKey(hmac[0], type, key, keySz);
                if (ret != 0) {
                    printf("wc_HmacSetKey failed for %s, ret = %d\n", label, ret);
                    goto exit;
                }

                ret = wc_HmacUpdate(hmac[0], bench_plain, bench_size);
                if (ret != 0) {
                    printf("wc_HmacUpdate failed for %s, ret = %d\n", label, ret);
                    goto exit;
                }
                
                ret = wc_HmacFinal(hmac[i], digest[i]);
                if (ret != 0) {
                    printf("wc_HmacFinal failed for %s, ret = %d\n", label, ret);
                    goto exit;
                }
            } /* for times */
            count += times;
...
exit_hmac:
	...
	printf("%.3f & ", label, (end-start)/(count)*1e6);
...
static void bench_hmac_custom_stream(int useDeviceID, int type, int digestSz,
                       const byte* key, word32 keySz, const char* label)
{
    WC_DECLARE_ARRAY(hmac, Hmac, BENCH_MAX_PENDING,
                     sizeof(Hmac), HEAP_HINT);
...
exit_hmac:
	...
	printf("%s-Custom(Stream) took %.3f us\n", label, (end-start)/(count)*1e6);
...
void bench_hmac_sha256_custom(int useDeviceID)
{
    WOLFSSL_SMALL_STACK_STATIC const byte key[] = {
                   0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b,
                   0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b,
    ...
    for (int i = 0 ; i < 13 ; i++) {
        bench_size = 1 << i;
        bench_hmac_custom(useDeviceID, WC_SHA256, WC_SHA256_DIGEST_SIZE, key, sizeof(key),
               "HMAC-SHA256");
    }
    printf("\n");
...
#ifdef WOLFSSL_SHA512
    WOLFSSL_SMALL_STACK_STATIC const byte key512[] = {
                   0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b,
                   0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b,
                   0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b,
                   0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b,
    ...
    for (int i = 0 ; i < 13 ; i++) {
        bench_size = 1 << i;
        bench_hmac_custom(useDeviceID, WC_SHA512, WC_SHA512_DIGEST_SIZE, key512, sizeof(key),
               "HMAC-SHA512");
    }
    printf("\n");
#endif
...
void bench_ecc_curve(int curveId)
{
    if (bench_all || (bench_asym_algs & BENCH_ECC_MAKEKEY)) {
    #ifndef NO_SW_BENCH
        printf("bench ecc makekey sw ECC_SECP256R1...\n");
    ...
    #ifndef NO_SW_BENCH
        printf("bench ecc sw ECC_SECP256R1...\n");
        bench_ecc(0, curveId);
    #endif
    #if defined(BENCH_DEVID)
        bench_ecc(1, curveId);
