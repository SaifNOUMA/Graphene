...
static int test_poly1305_ssl(int idx)
{
    const TESTDATA test = tests[idx];
    const u8 *in = test.input.data;
    size_t inlen = test.input.size;
    const u8 *key = test.key.data;
    const u8 *expected = test.expected.data;
    size_t expectedlen = test.expected.size;
    u8 out[16];

    if (sizeof(out) != expectedlen) {
        printf("Error: test_poly1305_ssl(%d): unexpected output size\n", idx);
        return 1;
    }
    ...
    if (mac_ctx == NULL) { return -1; }

    if (EVP_MAC_init(mac_ctx, key, 32, NULL) != 1) { return -1; }
    if (EVP_MAC_update(mac_ctx, in, inlen) != 1) { return -1; }
    if (EVP_MAC_final(mac_ctx, out, NULL, 16) != 1) { return -1; }

    if (memcmp(out, expected, expectedlen) != 0) {
        printf("Error: test_poly1305_ssl(%d): unexpected output\n", idx);
        return 1;
    }
...
static int test_poly1305_gcrypt(int idx)
{
    const TESTDATA test = tests[idx];
    const u8 *in = test.input.data;
    size_t inlen = test.input.size;
    ...
    if (sizeof(out) != expectedlen) {
        printf("Error: test_poly1305_gcrypt(%d): unexpected output size\n", idx);
        return 1;
    }

    gcry_mac_hd_t hd;
    gcry_error_t err;
    err = gcry_mac_open(&hd, GCRY_MAC_POLY1305, 0, NULL);
    if (err) { return -1; }
    err = gcry_mac_setkey(hd, key, 32);
    if (err) { return -1; }
    err = gcry_mac_write(hd, in, inlen);
    if (err) { return -1; }
    err = gcry_mac_read(hd, out, &outlen);
    if (err) { return -1; }
    gcry_mac_close(hd);

    if (memcmp(out, expected, expectedlen) != 0) {
        printf("Error: test_poly1305_gcrypt(%d): unexpected output\n", idx);
        return 1;
    }
...
static int test_poly1305_ref(int idx)
{
    const TESTDATA test = tests[idx];
    const u8 *in = test.input.data;
    size_t inlen = test.input.size;
    ...
    if (sizeof(out) != expectedlen) {
        printf("Error: test_poly1305_ref(%d): unexpected output size\n", idx);
        return 1;
    }

    memcpy(r, key, 16);
    memcpy(s, key+16, 16);
    poly1305_53(out, r, s, in, inlen);
...
int main(void)
{
    ...
    for (i = 0; i < sizeof(tests) / sizeof(tests[0]); i++) {
        if (test_poly1305_ssl(i) != 0) {
            ret = 1;
        }
        if (test_poly1305_gcrypt(i) != 0) {
            ret = 1;
        }
        if (test_poly1305_ref(i) != 0) {
            ret = 1;
        }
    }
