...
int bench_umac_sq(u32 bench_iterations, u32 datalen, u32 prime_bits, double *cpu_time);
int bench_umac_sq_batch(u32 bench_iterations, u32 batch_size, u32 datalen, u32 prime_bits, double *cpu_time);
...
int bench_umac_sq(u32 bench_iterations, u32 datalen, u32 prime_bits, double *cpu_time)
{
    ...
    for (u32 iter = 0 ; iter < bench_iterations ; iter++) {
        ...
        for (u32 block = 0; block < blocks-1; block++) {
            ...
            clock_gettime(CLOCK_MONOTONIC, &t0);
            mpz_add(umac_ctx.tmp_mpz, umac_ctx.a, umac_ctx.in_mpz);
            mpz_mul(umac_ctx.tmp_mpz, umac_ctx.tmp_mpz, umac_ctx.tmp_mpz);
            mpz_add(umac_ctx.out_mpz, umac_ctx.tmp_mpz, umac_ctx.out_mpz);
